...
from indel_utils import compute_insertion_probs, forward_deletions, remove_masked_tokens
...
class Diffusion(L.LightningModule):
    ...
    def _validate_configuration(self):
        ...
        if self.parameterization == "d3pm":
            assert self.T > 0
        if self.T > 0:
            assert self.parameterization in {"d3pm", "subs", "indel"}
    ...
    def _indel_parameterization(self, logits, xt):
        # we will not use the parameterization that
        # tries to predict x_0 direct from x_t (and then interpolates)
        # so mask_index is a valid output

        # only care about every other token;
        # these are predicted insertion distributions
        # logits = logits[:, 0::2]
        ...
        # TODO maybe forbid inserting at start and end if these are
        # outside of bos, eos
        ...
        padding_indices = xt == self.tokenizer.pad_token_id
        logits[padding_indices] = self.neg_infinity
        logits[padding_indices, xt[padding_indices]] = 0

        # also, do not allow inserting in the padding spans
        # we shift the padding indices to the left by 1
        # and force these outputs to be the padding token
        padding_indices_shifted_left = torch.cat(
            [padding_indices[:, 1:], torch.zeros_like(padding_indices[:, :1])], dim=1
        )
        logits[padding_indices_shifted_left] = self.neg_infinity
        logits[:, :, self.tokenizer.pad_token_id] *= (~padding_indices_shifted_left).to(
            logits.device
        )
    ...
    def _add_insertion_masks(self, x):
        x_ins = torch.full(
            (x.shape[0], x.shape[1] * 2 - 1),
    ...
    # TODO need to fix:
    # dealing with mask index; insertions in betweem padding tokens
    def _interleave_with_insertions(self, x, x_ins):
        device = x.device
        x = x.cpu()
        x_ins = x_ins.cpu()

        # print(self.mask_index, self.tokenizer.bos_token_id, self.tokenizer.eos_token_id)
        # print(x[100])
        # print(x_ins[100])
        ...
        i = torch.zeros(x.shape[0], dtype=torch.long)
        for j in range(res.shape[1]):
            if j % 2 == 0:
                ins_mask = x_ins[:, j // 2] != self.mask_index
                res[ins_mask][torch.arange(ins_mask.sum()), i[ins_mask]] = x_ins[
                    ins_mask, j // 2
                ]
                i += ins_mask
            else:
                res[torch.arange(res.shape[0]), i] = x[:, j // 2]
                i += 1
    ...
    def forward(self, x, sigma):
        ...
        if self.parameterization == "indel":
            x = self._add_insertion_masks(x)
        ...
        elif self.parameterization == "indel":
            return self._indel_parameterization(logits=logits, xt=x)
        return logits
    ...
    def _d3pm_loss(self, model_output, xt, x0, t):
        ...
        # logits of the correct x_0 tokens
        log_x_theta_at_x0 = torch.gather(model_output, -1, x0[:, :, None]).squeeze(-1)
        # logits of masked tokens (SUBS forces this to be -inf)
        log_x_theta_at_m = model_output[:, :, self.mask_index]
        # this would be all zeros
    ...
    def _indel_loss(self, model_output, xt, x0, t):
        ...
        # we only care about target_probs[:, 1:-1]
        target_probs, loss_mask = compute_insertion_probs(
            x0.cpu().numpy(),
            xt.cpu().numpy(),
            self.vocab_size,
            del_rate.cpu().numpy(),
        ...
        # model_output is computed for an input with insertion masks
        # the input is size (batch, xt_len*2-1)
        #   xt interleaved with xt_len-1 masks
        # thus we match the target_probs[1:-1] with the model_output[:, 1::2]
        ...
        # print("avg del prob", ins_logits[:, :, self.mask_index].exp().mean())
        # print("avg target del prob", target_probs[:, :, self.mask_index].mean())
        # print("avg token prob", ins_logits[:, :, 5:].exp().mean())
        # print("avg target token prob", target_probs[:, :, 5:].mean())

        return self.T * L_vb, loss_mask
    ...
    def q_xt(self, x, move_chance):
        ...
        if self.parameterization == "indel":
            ...
            xnp = x.cpu().numpy()
            xt = forward_deletions(
                xnp,
                move_chance.cpu().numpy(),
                self.tokenizer.bos_token_id,
                self.tokenizer.eos_token_id,
                self.tokenizer.pad_token_id,
            )
            return torch.from_numpy(xt).to(x.device)
    ...
    def _sample_prior(self, *batch_dims):
        if self.parameterization == "indel":
            state = torch.full(
                batch_dims[:-1] + (2,), self.tokenizer.pad_token_id, dtype=torch.int64
            )
            state[..., 0] = self.tokenizer.bos_token_id
            state[..., 1] = self.tokenizer.eos_token_id
            return state
        else:
    ...
    def _ddpm_update(self, x, t, dt):
        ...
        # this part is "overshooting" for x0 and then interpolating back
        # we simply sample from model_outputs then interleave with x

        if self.parameterization == "indel":
            if x.shape[1] >= self.config.model.length:
                # we can no longer afford to insert tokens
                return x
    ...
    def _sample(self, num_steps=None, eps=1e-5):
        ...
        if self.config.sampling.noise_removal:
            ...
            if self.sampler == "analytic":
                ...
                x = torch.from_numpy(_xnp).to(x.device)
        return x
    ...
    def _forward_pass_diffusion(self, x0):
        ...
        if self.T > 0:
            if self.parameterization == "indel":
                return self._indel_loss(model_output=model_output, xt=xt, x0=x0, t=t)
            else:
        ...
        if self.parameterization == "indel":
            assert False, "indel not supported for continuous time"
    ...
    def _loss(self, x0, attention_mask):
        ...
        if self.parameterization == "ar":
            ...
            if self.parameterization == "indel":
                loss, token_mask = self._forward_pass_diffusion(input_tokens)
            else:
